...
from app.forms import LoginForm, RegistrationForm, EditProfileForm, ForgotPasswordForm, PostForm
...
@login_required
def user(username):
	user = User.query.filter_by(username=username).first_or_404()	#first_or_404 works like first() method but if there is no results send 404 error	
	posts = [
...
def unfollow(username):
	...
	db.session.commit()
	flash('Your are not following {} since now.'.format(username))
	return redirect(url_for('user', username=username))
